...
|------|------|
| 功能块名称 | C_CP13 |
| 功能描述 | Speed Reference Compound at Select(4 Part)（四选一速度参考复合） |
| 最后修改 | 2016.01.05 |
| 作者 | ShiChunLiang |
| 页数 | 1页（2个程序段） |
...
C_CP13是一个速度参考复合功能块，用于在四个速度参考源之间进行选择和切换。该功能块支持四个独立的速度参考输入，根据选择信号自动切换输出对应的速度参考值，并输出对应的运行状态。
...
**功能描述**: 根据四个部分的选择信号确定总运行状态

**输入条件**:
| 信号名称 | 信号描述 | 信号类型 | 触发值 |
|----------|----------|----------|--------|
| RUN1~RUN4 | 运行状态1~4 | BOOL | TRUE/FALSE |
| SEL1~SEL4 | 选择信号1~4 | BOOL | TRUE |
...
**功能实现**: 
四个部分的选择逻辑并联，任一部分被选中时输出RUN为TRUE。
...
| SEL1~SEL4 | 选择信号 | BOOL | TRUE/FALSE |
...
### 选择条件
- **第n部分选中**: RUNn = TRUE AND SELn = TRUE (n=1~4)
...
| SEL1~SEL4 | 选择信号1~4 | BOOL | 各部分选择控制 |

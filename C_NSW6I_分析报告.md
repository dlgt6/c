...
|------|------|
| 功能块名称 | C_NSW6I |
| 功能描述 | Numerical Changeover Switch, 6 Branch Selector(INT type)（数值切换开关，6分支选择器，INT类型） |
| 最后修改 | 2017.10.12 |
| 作者 | Hu Jing Qi |
| 页数 | 2页 |
...
C_NSW6I 是一个6分支数值切换开关功能块，用于根据开关设置选择六个INT类型输入值中的一个。该功能块支持优先级选择，I1优先级最高，I6最低。

## 思维导图

```mermaid
graph TD
    A[C_NSW6I 6分支切换开关] --> B[输入模块]
    A --> C[处理模块]
    A --> D[输出模块]
...
**功能实现**: 
使用MOVE功能块，根据I1-I6的开关状态，选择相应的输入值输出到Y。当所有开关都为FALSE时，输出默认值0.1。
...
### 主要功能
1. **分支选择**: 根据优先级选择六个输入值中的一个

## 关键信号说明

| 信号名称 | 信号描述 | 信号类型 | 用途 |
|----------|----------|----------|------|
| I1-I6 | 分支开关设置 | BOOL | 分支选择 |
| X1-X6 | 分支输入 | INT | 分支输入值 |
| Y | 输出 | INT | 选择输出值 |

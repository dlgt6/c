...
|------|------|
| 功能块名称 | C_NSW3D |
| 功能描述 | Numerical Changeover Switch, 3 Branch Selector(DINT type)（数值切换开关，3分支选择器，DINT类型） |
| 最后修改 | 2015.11.20 |
| 作者 | Shi Chun Liang |
| 页数 | 2页 |
...
C_NSW3D 是一个3分支数值切换开关功能块，用于根据开关设置选择三个DINT类型输入值中的一个。该功能块支持优先级选择，I1优先级最高，I2次之，I3最低。

## 思维导图

```mermaid
graph TD
    A[C_NSW3D 3分支切换开关] --> B[输入模块]
    A --> C[处理模块]
    A --> D[输出模块]
...
| X1 | 分支1输入 | DINT | 分支1输入值 |
| X2 | 分支2输入 | DINT | 分支2输入值 |
| X3 | 分支3输入 | DINT | 分支3输入值 |
| Y | 输出 | DINT | 选择输出值 |

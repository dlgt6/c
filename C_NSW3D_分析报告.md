...
|------|------|
| 功能块名称 | C_NSW3D |
| 功能描述 | Numerical Changeover Switch, 3 Branch Selector(DINT type)（数值选择开关，3分支选择器，DINT类型） |
| 最后修改 | 2015.11.20 |
| 作者 | Shi Chun Liang |
| 页数 | 1页 |
...
C_NSW3D 是一个3分支数值选择开关功能块，用于根据选择信号在三个DINT类型输入值之间切换输出。选择信号I1、I2、I3按优先级顺序选择对应的输入值输出。
...
| 信号名称 | 信号描述 | 信号类型 | 用途 |
|----------|----------|----------|------|
| X1 | 输入值1 | DINT | 选择输入1（最高优先级） |
| X2 | 输入值2 | DINT | 选择输入2（次优先级） |
| X3 | 输入值3 | DINT | 选择输入3（最低优先级） |
| I1 | 选择信号1 | BOOL | 选择输入1 |
| I2 | 选择信号2 | BOOL | 选择输入2 |
| I3 | 选择信号3 | BOOL | 选择输入3 |
| Y | 输出 | DINT | 选择输出 |
